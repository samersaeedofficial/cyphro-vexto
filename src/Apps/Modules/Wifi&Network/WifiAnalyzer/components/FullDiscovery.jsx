...
const FullDiscovery = ({ onBack, onStartAttack }) => {
  ...
  return (
    <div className="w-full min-h-screen relative space-y-8 animate-in fade-in duration-700">
      ...
      {/* ─── Terminating Loading Overlay ─── */}
      <AnimatePresence>
        {isExitLoading && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="absolute inset-0 z-[60] bg-slate-950/60 backdrop-blur-2xl flex flex-col items-center justify-center text-center p-12 rounded-[2.5rem] border border-red-500/10"
          >
             <motion.div 
               animate={{ rotate: -360 }}
               ...
               className="mb-8"
             >
                <RotateCcw className="w-20 h-20 text-red-500/50" />
             </motion.div>
             <div className="space-y-4">
                <h3 className="text-3xl font-black text-white tracking-tighter">TERMINATING DISCOVERY</h3>
                <div className="flex flex-col items-center gap-2">
                   <div className="flex items-center gap-2 text-red-400 font-mono text-xs uppercase tracking-[0.3em]">
                      <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
                      Wiping Volatile Memory
                   </div>
                   <p className="text-slate-500 text-[10px] font-mono max-w-xs">
                      Securely clearing {networks.length} identified network profiles from system discovery cache...
                   </p>
                </div>
             </div>
      ...
      <AnimatePresence>
        {showCancelModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowCancelModal(false)} className="absolute inset-0 bg-slate-950/80 backdrop-blur-md" />
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-md bg-slate-900 border border-white/10 rounded-[2.5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] overflow-hidden">
