...
const WifiHistory = () => {
  ...
  useEffect(() => {
    ...
    if (showCalendar || showFilterDropdown) {
      document.addEventListener("mousedown", handleClickOutside);
    }
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, [showCalendar, showFilterDropdown]);
  ...
  const historyData = useMemo(() => {
    if (activeFilter === "all") return rawHistoryData;
    return rawHistoryData.filter(item => item.type === activeFilter);
  }, [rawHistoryData, activeFilter]);

  const FILTER_OPTIONS = useMemo(() => [
    { id: "all", label: "All Records", icon: Database },
    { id: "discovery", label: "Discovery", icon: Search },
    { id: "handshake", label: "Handshakes", icon: Lock },
    { id: "wps", label: "WPS Attacks", icon: Zap }
  ], []);
  ...
  return (
    <section className="space-y-6">
        ...
        <div className="flex items-center gap-4 relative">
          ...
          <div className="relative" ref={filterRef}>
            <button
              ...
              className={`p-3 rounded-2xl border transition-all flex items-center gap-2 group ${showFilterDropdown ? 'bg-cyan-500/20 border-cyan-500/30 text-cyan-400' : 'bg-slate-900/60 border-white/5 text-slate-400 hover:text-white'}`}
            >
              <Filter className={`w-4 h-4 transition-transform ${showFilterDropdown ? 'scale-110' : ''}`} />
              {activeFilter !== 'all' && <span className="text-[10px] font-black uppercase tracking-widest pr-1">{activeFilter}</span>}
            </button>
            ...
            <AnimatePresence>
              {showFilterDropdown && (
                <motion.div
                  ...
                  className="absolute top-full right-0 mt-4 z-[60] w-64 p-2 rounded-3xl bg-slate-900/95 backdrop-blur-3xl border border-white/10 shadow-[0_30px_60px_rgba(0,0,0,0.5)]"
                >
                  <div className="px-4 py-3 border-b border-white/5 mb-2">
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Filter Logs</span>
                  </div>
                  {FILTER_OPTIONS.map((option) => (
                    <button
                      key={option.id}
                      onClick={() => { setActiveFilter(option.id); setShowFilterDropdown(false); }}
                      className={`w-full flex items-center justify-between px-4 py-3 rounded-2xl text-xs font-bold transition-all ${activeFilter === option.id ? 'bg-cyan-500/10 text-cyan-400' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
                    >
                      <div className="flex items-center gap-3">
                        <option.icon className={`w-4 h-4 ${activeFilter === option.id ? 'text-cyan-400' : 'text-slate-500'}`} />
                        {option.label}
                      </div>
                      {activeFilter === option.id && <div className="w-1.5 h-1.5 rounded-full bg-cyan-400 shadow-[0_0_8px_#22d3ee]" />}
                    </button>
                  ))}
                </motion.div>
              )}
      ...
      <div className="relative rounded-[2.5rem] bg-slate-900/40 backdrop-blur-2xl border border-white/5 shadow-2xl overflow-hidden">
        <div className="max-h-[550px] overflow-y-auto p-6 md:p-10 custom-scrollbar scroll-smooth">
          ...
          {historyData.length === 0 && (
            <div className="py-20 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-slate-500 font-mono text-sm uppercase tracking-widest">No Activity Records Found</p>
            </div>
          )}
        </div>

        {/* Bottom Fade-out Overlay for better scroll depth perception */}
